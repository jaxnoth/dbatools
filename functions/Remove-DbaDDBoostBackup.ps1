function Remove-DbaDDBoostBackup {
    ...
    BEGIN {
        ...
        $fileExe = Join-SomePath $fileExe "bin\ddbmexptool.exe"

        if ($PSCmdlet.ShouldProcess("$ComputerName", "Cleanup backup history")) {
            try {
                $programResult = Invoke-Program -ComputerName $ComputerName -Path $fileExe -Authentication CredSSP -Credential $Credential -ArgumentList $Args -EnableException $True
